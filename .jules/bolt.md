...
## 2026-03-02 - Duplicated Operations and Batch Sizes
**Learning:** In bash scripts, especially setup scripts that evolve over time, it's easy to accidentally duplicate expensive operations like `apt update`, repository additions, or package installations. Here, the entire repository configuration block and a 1984-package `xargs` installation were duplicated.
**Action:** Always audit setup scripts for duplicated blocks of code. Ensure `apt update` is run only when strictly necessary, and consolidate duplicate `xargs` calls while maximizing batch size (e.g. 3000) to minimize package manager invocation overhead.
