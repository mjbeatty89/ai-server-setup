...
## 2025-02-12 - Prevent Implicit apt Updates & Optimize Substring Matches
**Learning:** `add-apt-repository` triggers implicit `apt update` calls if the `-n` flag isn't used, creating major bottlenecks when used sequentially. Additionally, filtering strings from long package arrays with `grep -vE` is far more expensive when those dependencies have already been installed (e.g. `$ESSENTIALS`). Using a temporary file alongside `grep -vFxf` for the exact matches correctly solves the substring match overhead and skips redundant package manager execution.
**Action:** Always append the `-n` flag on `add-apt-repository` calls in consolidated setup scripts, and filter already-installed sub-lists (like `$ESSENTIALS`) before passing packages to `xargs` to massively reduce network and execution overhead.
